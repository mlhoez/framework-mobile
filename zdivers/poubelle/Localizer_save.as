package com.ludofactory.mobile.core
{
	...
	| Auteur      : Maxime Lhoez
	| Création    : 2 mai 2013
	| Description : This is the class handling the localization technique for the application.
	...
	public class Localizer
	{
		...
		private function parseLanguage(language:String):void
		{
			...
			if( _languagesLoaded.hasOwnProperty(language) )
				return;
			
			var fileStream:FileStream = new FileStream();
			var languageFile:File = File.applicationDirectory.resolvePath( "lang/" + language + ".csv" );
			...
			if( languageFile.exists )
			{
				...
				for(var i:int = len; i >= 0; i--)
				{
					...
					csv += value.split("=")[0] + "#" + value.split("=")[1] + "\n";
				}
				
				var file:FileReference = new FileReference();
				var bytes:ByteArray = new ByteArray();
				bytes.writeMultiByte(csv, "iso-8859-1");
				file.save(bytes, String(language + ".csv"));
...
//------------------------------------------------------------------------------------------------------------
		...
		{
			if( _languagesLoaded[_lang].translation.hasOwnProperty( key ) )
