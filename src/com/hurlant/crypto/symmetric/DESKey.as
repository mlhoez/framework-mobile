/*
 * This is a modified version of the DESKey class from the AS3 Crypto Library:
...
package com.hurlant.crypto.symmetric
{
	...
	public class DESKey implements ISymmetricKey
	{
		 ...
		 */
		protected function generateWorkingKey(encrypting:Boolean, key:ByteArray, off:int):Array
		{
		 ...
		 * the DES engine.
		 */
		protected function desFunc(wKey:Array, inp:ByteArray, inOff:int, out:ByteArray, outOff:int):void
		{
			var work:int;
			var right:int;
			var left:int;
		...
		{
			var i:int = 0;
			var len:int = encKey.length;
			for(i = 0; i < len; i++) { encKey[i] = 0; }
			encKey = null;
			
			len = decKey.length;
			for(i = 0; i < len; i++) { decKey[i] = 0; }
			decKey = null;
