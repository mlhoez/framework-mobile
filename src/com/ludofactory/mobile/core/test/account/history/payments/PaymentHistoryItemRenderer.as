...
package com.ludofactory.mobile.core.test.account.history.payments
{
	 ...
	 */	
	public class PaymentHistoryItemRenderer extends FeathersControl implements IGroupedListItemRenderer
	{
		/**
		...
		public function PaymentHistoryItemRenderer()
		{
			super();
		}
		...
		override protected function initialize():void
		{
			...
			_title = new Label();
			addChild(_title);
			_title.textRendererProperties.textFormat = Theme.paymentIRTitleTextFormat;
		}
		...
		protected function commitData():void
		{
			if(this._owner)
			{
				if( _data )
				{
					...
					_title.text = _data.date + " - " + _data.type + " - " + _data.offer;
		...
		protected function layout():void
		{
			...
			if( _groupIndex != -1 && owner/* && owner.dataProvider && (owner.dataProvider.data.length - 1) == _groupIndex*/ && (owner.dataProvider.data[_groupIndex].children.length - 1) == _itemIndex)
			{
		...
		protected var _data:PaymentHistoryData;
		...
		public function set data(value:Object):void
		{
			...
			this._data = PaymentHistoryData(value);
