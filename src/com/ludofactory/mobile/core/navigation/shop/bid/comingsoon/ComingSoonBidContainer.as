...
*/
package com.ludofactory.mobile.core.navigation.shop.bid.comingsoon
{
	...
	public class ComingSoonBidContainer extends FeathersControl
	{
		/**
		...
		public function ComingSoonBidContainer()
		{
			super();
		}
		...
		override protected function initialize():void
		{
			...
			if( AirNetworkInfo.networkInfo.isConnected() )
				Remote.getInstance().getComingSoonBids(onGetComingSoonBidsSuccess, onGetComingSoonBidsFailure, onGetComingSoonBidsFailure, 2, AbstractEntryPoint.screenNavigator.activeScreenID);
			else
...
//------------------------------------------------------------------------------------------------------------
		...
		private function onGetComingSoonBidsSuccess(result:Object):void
		{
			...
			{
				var i:int = 0;
				var len:int = (result.enchere_avenir as Array).length;
				var arr:Array = [];
				for( i; i < len; i++)
					arr.push( new ComingSoonBidItemData(result.enchere_avenir[i]) );
				_list.dataProvider = new ListCollection( arr );
				_list.selectedIndex = -1;
			});
		}
		
		private function onGetComingSoonBidsFailure(error:Object = null):void
		{
			_list.onRefreshComplete();
