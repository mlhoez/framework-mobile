...
package com.ludofactory.mobile.navigation.sponsor
{
	...
	public class SponsorHomeScreen extends AdvancedScreen
	{
		...
		override protected function initialize():void
		{
			...
			_rewardValueLabel.text = "hihi"; // always a String well formatted : whether "30 €" or "100 000"
			...
			_byFilleulLabel.text = _("PAR FILLEUL");
			...
			_facebookButton = ButtonFactory.getFacebookButton(_("Partager"), ButtonFactory.FACEBOOK_TYPE_SHARE, StringUtil.format(_("Mon code parrain : {0} !"), MemberManager.getInstance().id),
					"",
		...
		override protected function draw():void
		{
			if( isInvalid(INVALIDATION_FLAG_SIZE) )
			{
				...
				else
				{
					...
					_emailButton.validate();
					_friendsImage.y = ((actualHeight) - (_emailButton.height * 2 + _friendsImage.height * 0.4 + scaleAndRoundToDpi(GlobalConfig.isPhone ? 20 : 50)) ) * 0.5;
...
//	Handlers
		...
		private function onEmailSelected(event:Event):void
		{
			...
			{
				this.advancedOwner.replaceScreen( ScreenIds.SPONSOR_INVITE_SCREEN );
			}
			else
			{
		...
		private function onMyFriendsSelected(event:Event):void
		{
			if( MemberManager.getInstance().isLoggedIn() )
			{
				this.advancedOwner.replaceScreen( ScreenIds.SPONSOR_FRIENDS_SCREEN );
			}
			else
			{
				advancedOwner.replaceScreen( ScreenIds.REGISTER_SCREEN );
...
//	Facebook
		...
		private function onRefreshPublicationData(event:Event):void
		{
			...
			_facebookButton.refreshPublicationData(StringUtil.format(_("Mon code parrain : {0} !"), MemberManager.getInstance().id),
					"",
					_("Devenez mes filleuls en vous inscrivant avec ce code et gagnez des tas de bonus !"),
					_("http://www.ludokado.com/"),
					StringUtil.format(_("http://img.ludokado.com/img/frontoffice/{0}/mobile/publication/pyramid.jpg"), LanguageManager.getInstance().lang));
